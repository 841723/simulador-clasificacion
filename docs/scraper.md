...
# Scraping incremental (detecta automáticamente la primera jornada incompleta)
python sofascore.py
...
La auto-detección busca la primera jornada donde no todos los partidos están finalizados, menos 1 de buffer.
...
2. **Calcula la jornada de inicio** (si no se especifica `--from-jornada`): busca en la BD la primera jornada con partidos no finalizados.
3. **Lanza Playwright** (Chromium headless) para hacer peticiones a la API de SofaScore.
4. Por cada jornada en el rango:
