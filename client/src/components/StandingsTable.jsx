...
export default function StandingsTable() {
  ...
  const [size, _] = useState(sizeOptions[0]);
  ...
  function getZoneClass(position) {
    ...
    if (position >= 19) return 'border-l-4 border-red-400';
    return '';
  }
      ...
      <div className='max-w-4xl mx-auto px-4 py-6'>
          ...
          {selected.length > 0 && (
              <p className='text-xs text-gray-500 mt-2'>
                  Equipos seleccionados: {selected.join(", ")}. También se
                  resaltan en la vista por equipo.
              </p>
          )}
      </div>
  );
